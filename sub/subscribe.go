...
func SubscribeContract(ctx context.Context, wsUrl string, wg *sync.WaitGroup) {
	defer wg.Done()

	// 连接WebSocket
	contractConn, _, err := websocket.DefaultDialer.Dial(wsUrl, nil)
	if err != nil {
...
func SubscribeSpot(ctx context.Context, wsUrl string, wg *sync.WaitGroup) {
	defer wg.Done()

	// 连接WebSocket
	spotConn, _, err := websocket.DefaultDialer.Dial(wsUrl, nil)
	if err != nil {
